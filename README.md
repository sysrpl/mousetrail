...
Already present on a stock Linux Mint install — nothing to `pip install`:

- X11 (a Wayland session will not work — this uses the X RECORD extension)
...
| `COLOR` | white | RGB, each channel 0–1 |
...
`END`, `PGUP`, `PGDN`, `F1`–`F12`, and arrows as `↑ ↓ ← →`. Edit that dict to
...
- **The toggle key still reaches your apps.** The overlay watches input rather
  ...
  `"Scroll_Lock"` or `"Pause"` — almost nothing else binds those.
- **Held keys show once.** Auto-repeat is suppressed, so holding Backspace
  produces one `[BKSP]` cap rather than a stream of them.
- **Multi-monitor:** the window spans the whole desktop; caps are centered on
...
Click-through comes from an empty input region set on the GTK widget, which
GTK re-applies across realize and resize, so the overlay cannot start
swallowing clicks.
